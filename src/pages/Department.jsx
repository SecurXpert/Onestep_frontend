...
// import eyetip1 from '../assets/eyetip1.png';
// import eyetip2 from '../assets/eyetip2.png';
// import eyetip3 from '../assets/eyetip3.png';
...
//       image: eyetip1, // Replace with relevant ophthalmology image
//     },
//     {
//       id: 2,
...
//       image: eyetip2, // Replace as needed
//     },
//     {
//       id: 3,
...
//       image: eyetip3, // Replace as needed
//     },
//   ],
//   nurse: [
//     {
...
//   //     try {
//   //       setLoading(true);
//   //       let url = `http://192.168.0.170:8000/doctors/by-specialization/${encodeURIComponent(specialtyName)}`;
//   //       if (searchParams.area) {
//   //         url = `http://192.168.0.170:8000/doctors/by-specialization/area_spec/?specialization_name=${encodeURIComponent(specialtyName)}&area=${encodeURIComponent(searchParams.area)}`;
//   //       }
...
//       }
//       const data = await response.json();
//       console.log('Fetched doctors:', data); // Debug: Log the API response
//       setDoctors(data);
//       setLoading(false);
//     } catch (err) {
//       setError(err.message);
...
//   // Scroll to top when specialtyName changes
//   useEffect(() => {
//     window.scrollTo(0, 0);
//   }, [specialtyName]);
...
//           <button
//             type="submit"
//             className={`w-full md:w-auto px-6 py-2 rounded-lg text-white transition-colors ${isSearchDisabled ? 'bg-gray-400 cursor-not-allowed' : 'bg-custom-blue hover:bg-blue-700'
//               }`}
...
const Department = () => {
  ...
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [pincodeError, setPincodeError] = useState(null); // New state for pincode error
  ...
  const handleSearch = async (e) => {
    ...
    // Validate pincode before proceeding with the search
    if (area && (area.length !== 6 || isNaN(area))) {
    ...
    if (searchTerm) {
      try {
        ...
        setDoctors(data);
        setLoading(false);
        setPincodeError(null); // Clear any previous pincode error
      } catch (err) {
        setError(err.message);
        setDoctors([]);
        setLoading(false);
  ...
  const handleInputChange = (e) => {
    ...
    // Validate pincode as the user types
    if (name === 'area') {
  ...
  {pincodeError && (
    <p className="text-red-600 text-sm mt-2">{pincodeError}</p>
  )}
</div>
