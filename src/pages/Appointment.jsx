...
const Appointment = () => {
  ...
  const fetchRelatedDoctors = async () => {
    ...
    try {
      const response = await fetchWithAuth(
        `http://192.168.0.120:8000/slots/similar-doctors?specialization=${encodeURIComponent(
 ...
 const fetchAvailableSlots = async (date) => {
    ...
    try {  
      const response = await fetchWithAuth(
        `http://192.168.0.120:8000/slots/available-slots?preferred_date=${date}&doctor_id=${id}`,
        {
          method: 'GET',
          headers: { 'Content-Type': 'application/json' },
        }
...
const handleBook = async () => {
  ...
  try {
    ...
    const response = await fetchWithAuth('http://192.168.0.120:8000/appointments/book', {
      method: 'POST',
      body: formData,
    });
