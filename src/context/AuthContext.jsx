...
export const AuthProvider = ({ children }) => {
  ...
  const register = async (name, number, dob, email, password) => {
    try {
      const response = await fetch('http://192.168.0.123:8000/patient/signup', {
        method: 'POST',
  ...
  const login = async (email, password) => {
    try {
      const response = await fetch('http://192.168.0.123:8000/patient/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email,
      ...
      if (response.ok) {
        ...
        const profileResponse = await fetchWithAuth('http://192.168.0.123:8000/profile/me', {
          method: 'GET',
        });
